...
class ConfigTests(unittest.TestCase):
    ...
    def test_set_active_and_enabled(self):
        with isolated_home():
            ...
            cfg = set_active("deepseek")
            self.assertEqual(cfg["active"], "deepseek")
            cfg = set_enabled(True)
            self.assertTrue(cfg["enabled"])
            again = load()
            self.assertEqual(again["active"], "deepseek")
    ...
    def test_builtin_override_via_overrides(self):
        with isolated_home():
            ...
            update_override("deepseek", {"api_key": "sk-real", "model": "deepseek-v4"})
